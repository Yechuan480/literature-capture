...
| **批量提取** | 「提取表格」对**全部**未提取截图批量跑 PP-TableMagic / OCR / 可选 AI（跨文献累加） |
...
- （推荐）PaddleX / PP-TableMagic：批量提取时的表格识别（首次运行会下载多 GB 模型）
...
内存建议 ≥ 8GB。若暂不安装 Paddle，应用仍可运行（提取回退 Tesseract / RapidOCR）。
...
其他快捷键：`←`/`→` 翻页 · `T`/`Shift+T` 跳 Table · `R`/`Shift+R` 旋转 · `Esc` 取消框选 
...
| GET | `/api/detect/status` | Paddle 识别就绪状态（内部/调试） |
