...
def ocr_status(settings: Settings | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "hint": (
            ...
            else "未检测到 tesseract，将使用 RapidOCR 后备。建议: brew install tesseract tesseract-lang"
        ),
    }
...
def extract_table(
    ...
    *,
    ...
    force_engine: None|auto|img2table_tesseract|rapidocr|ai
      - ai: skip local OCR, vision only
      - rapidocr / img2table_tesseract: force that path (AI only if use_ai)
    ...
    if forced == "ai":
        ...
        if engine == "img2table_tesseract" and tesseract_available():
