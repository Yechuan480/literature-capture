...
3. 右侧详情：改阅读状态、标签、笔记 → **保存**；**打开阅读** / **表格截取**。
...
1. **框选翻译**  
   - 点 **框选翻译** → 在页面上拖出区域 → **译选区**。  
   - 优先用 PDF 内嵌文字（pdfplumber crop）；文字过少时用选区截图走视觉翻译。  
   - 右侧面板显示译文，可 **复制** / **关闭**；`Esc` 退出框选。

2. **全文翻译**  
   - 点 **全文翻译** 提交后台任务（按页提取 → 机译 → 写入 reflow PDF）。  
   - 完成后出现 **打开译稿**；译稿文件名为 `{原名stem}.zh-CN.pdf`，与原文同目录（`pdfs/`）。  
   - 已有译稿时再次点击会直接复用；**Shift+全文翻译** 可强制重译。  
   - 扫描件无可提取文本时全文任务会失败；区域翻译仍可依赖视觉。
